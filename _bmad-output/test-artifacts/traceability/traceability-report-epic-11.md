...
stories_pending: []
gate_decision: 'IN-PROGRESS (7/7 stories done; epic-level gate pending retrospective)'
---
...
| **11.4** Persona journeys | 7 tests | **7 ✅** (×3 engines = 21 runs) | **7 ✅** | None — pure test-app harness + PW-UI locks, no lib change. All 7 persona IDs green on chromium/firefox/webkit (21/21, no flakes, no skips). 3 test-app debug toggles added. R-P2-1 (last HIGH cluster) covered by C2. Independent Opus review: Approve 0H/0M/3L (all informational, no change). → **done** |
...
| **11.7-D-001** | G#25 — Resume example compiles + completes a resume against MinIO | `tests/integration/docs/resume-example.test.ts:63,116` | ✅ GREEN (compile + MinIO round-trip) | Compile leg: README resume block type-checks against `.d.mts` (always on). MinIO leg (REVIEW FIX M1): a real **programmatic two-session crash-resume** — session 1 PUTs part 1 then stops before complete; session 2 calls `uploadMultipart` with `resumeFrom` + `reconcileCompletedParts` → uploads ONLY part 2 → completes → HEAD/GET asserts full 6 MiB assembly + `uploadedThisSession === [2]`. Verified GREEN with `MINIO_REQUIRED=1`. Skips gracefully when MinIO is down. (The literal README block can't be run — its session 1 completes the upload, so a real resume needs the programmatic path; honest-scope Pattern 3.) |
...
**Story 11.7 coverage: 10/10 implemented GREEN + 1 DEFERRED tracked = 11/11 IDs.** 10 IDs are GREEN (E2E-002, E2E-003, X-001, X-002, INT-001, INT-002, D-001 compile + MinIO round-trip, D-002, D-003); 11.7-E2E-001 is DEFERRED to Epic 12 (`test.fixme` placeholder, no effort consumed). **No lib fix surfaced** — all locks are surface-area/contract codifications. Triptyque green: `pnpm turbo build` + (core 204 + adapters 55 + integration 23 + PW-Lib 10 green / 1 skip) + `pnpm turbo typecheck` 5/5. **3 Epic 13 candidates surfaced inline:** (1) `simpleHttpUpload` cross-browser streaming TRANSMISSION over HTTP/1.1 (E2E-002, R-P2-4 / Decision D1); (2) pre-flight `InitiateUploadError` guard for >1024-char keys (INT-002, R-P2-14); (3) per-engine buffered fallback / HTTP/2 negotiation for request streams (E2E-002).

**Independent Opus code-review (2026-06-02): Changes-Requested → all addressed → ✅ Approve.** 1 MEDIUM + 2 LOW:
- **M1 (FIXED):** 11.7-D-001's "runs end-to-end against MinIO" leg was a tautology (`compileBlock` + `expect(reachable).toBe(true)`) — green even though it drove no resume. Replaced with a genuine programmatic two-session crash-resume against MinIO (verified with `MINIO_REQUIRED=1`). The README block can't be run literally (session 1 completes the upload), so the programmatic path is the honest faithful test for G#25.
- **L1 (FIXED — turned out substantive):** the E2E-002 transmission lock could pass for the wrong reason. On investigation the probe ran from `about:blank` and fetched cross-origin to `:5173`, so it was passing purely on CORS NetworkErrors, not the HTTP/1.1 streaming gap. Fix: probe navigates to the endpoint ORIGIN (same-origin) + a buffered-PUT control proves reachability + asserts the failure is streaming-specific. Now the transmission gap is genuinely locked (FF/WebKit accept buffered, reject streamed over HTTP/1.1).
- **L2 (no change — concur):** one INT-001 sub-case asserts `encodeURIComponent` (a JS builtin); reviewer advised no change. Left as documentation; not counted as adapter coverage.

Status → **done**.

### 2.8 — Story 11.5 (R-P2-3 TECH HIGH + R-P2-9 BUS MEDIUM, 13 tests × 3 engines)

All specs run in the `lib` Playwright project; cross-browser is achieved by launching Chromium / Firefox / WebKit inside each test (mirrors Epic 10 `*-cross-browser` / `*-support-matrix` lib specs). PUT-level chaos is injected via `context.route` at the browser network layer — the browser PUTs directly to MinIO `:9000`, so the Fastify `/api/chaos` endpoint cannot see those requests. Set `CHAOS_ENGINES=chromium` to restrict the matrix during development.

| Test ID | Brainstorming origin | Spec path | Status | Notes |
|---|---|---|---|---|
| **11.5-E2E-001** | C#1 — 30% PUT loss completes via retries | `tests/e2e/lib/chaos-intermittent.spec.ts` | ✅ GREEN ×3 | Deterministic transient injection (every part's 1st PUT fails) + full-object HEAD on MinIO |
| **11.5-E2E-002** | C#3 — 8s offline exhausts default schedule | `tests/e2e/lib/chaos-intermittent.spec.ts` | ✅ GREEN ×3 | Sustained outage → `MaxRetriesExceededError` (3 attempts, ~300ms ≪ 8s); tuning need captured (Epic 13) |
| **11.5-E2E-003** | C#4 — part-transport truncation | `tests/e2e/lib/chaos-intermittent.spec.ts` | ✅ GREEN ×3 | Single attempt → raw `PartUploadError` (part phase; AC-named variant) |
| **11.5-E2E-004** | C#5 — missing ETag in 200 OK | `tests/e2e/lib/chaos-intermittent.spec.ts` | ✅ GREEN ×3 | Retried → `MaxRetriesExceededError` (PartUploadError retry-exhaustion wrapper, partNumber-tagged) |
| **11.5-E2E-005** | C#6 — garbage ETag → InvalidPart | `tests/e2e/lib/chaos-intermittent.spec.ts` | ✅ GREEN ×3 | Complete phase → `CompleteUploadError`; part-phase proof via callback attempt counts |
| **11.5-E2E-006** | C#7 — parts 2 & 3 fail simultaneously | `tests/e2e/lib/chaos-simultaneous.spec.ts` | ✅ GREEN ×3 | Independent per-part retry state (`partAttempts` {1:1,2:2,3:2}) + full-object HEAD |
| **11.5-E2E-007** | C#8 — abort during backoff (raceFirst) | `tests/e2e/lib/chaos-simultaneous.spec.ts` | ✅ GREEN ×3 | 10s spaced backoff; abort latency < 2s ≪ 10s proves `raceFirst` not `race` |
| **11.5-E2E-008** | C#12 — slow transport, no premature timeout | `tests/e2e/lib/chaos-degraded.spec.ts` | ✅ GREEN ×3 | ~400ms/PUT latency; completes (no hardcoded client timeout) |
| **11.5-E2E-009** | C#13 — abort responsive under high latency | `tests/e2e/lib/chaos-degraded.spec.ts` | ✅ GREEN ×3 | 5s PUT latency; abort during first PUT → `AbortError`, latency < 2s |
| **11.5-E2E-010** | C#15 — slow-loris part still completes | `tests/e2e/lib/chaos-degraded.spec.ts` | ✅ GREEN ×3 | ~3s trickle; completes. Epic 13 candidate: `partTimeout` option |
| **11.5-E2E-011** | C#18 — abort during `/initiate` | `tests/e2e/lib/chaos-abort-timing.spec.ts` | ✅ GREEN ×3 | `AbortError`/`InitiateUploadError`; orphan-multipart cleanup is an Epic 13 candidate |
| **11.5-E2E-012** | C#19 — abort between parts | `tests/e2e/lib/chaos-abort-timing.spec.ts` | ✅ GREEN ×3 | Callback-side trigger; `partsCompletedViaCallback===1`, never UploadCompleted |
| **11.5-E2E-013** | C#20 — abort during `/complete` | `tests/e2e/lib/chaos-abort-timing.spec.ts` | ✅ GREEN ×3 | `AbortError`/`CompleteUploadError`; no clean late-stage recovery (Epic 13) |
...
**Story 11.4 coverage: 7/7 = 100% ✅** (21 runs across Chromium + Firefox + WebKit, no flakes, no skips — D3 stabilization satisfied first try). No lib change — pure test-app harness + PW-UI locks. Triptyque (build + core 204 + adapters 55 + integration 23 + typecheck 5/5) green; chaos-isolation re-audited 150/150 (personas bypass the Fastify chaos endpoint via `page.route`, so they don't poison the per-session audit). 1 Epic 13 candidate surfaced (C2 stale-uploadId auto-re-init). **Independent Opus code-review: ✅ Approve 0H/0M/3L** (L1 A1 error alternation includes unreachable `InitiateUploadError`; L2 A2 non-Chromium slow-sign is the documented D3 compromise; L3 pre-existing `findObjectKey` duplication — all informational, reviewer advised no change). Reviewer traced B5/C2/A4/A1/B6/B1 against lib internals and confirmed no tautologies. Status → **done**.
...
| G#25 — Resume example doctest | R-P2-14 | 11.7-D-001 | ✅ GREEN (compile + real MinIO two-session crash-resume; review fix M1) |
...
### 3.7 — R-P2-3 / R-P2-9 — STORY 11.5 COVERAGE

| Scenario | Risk | Test ID(s) | Status |
|---|---|---|---|
| C#1 — 30% intermittent PUT loss → completes via retries | R-P2-3 (TECH, HIGH) | 11.5-E2E-001 (×3) | ✅ GREEN |
| C#3 — 8s offline window exhausts default schedule | R-P2-3 | 11.5-E2E-002 (×3) | ✅ GREEN — tuning need / Epic 13 |
| C#4 — partial-response truncation → `PartUploadError` | R-P2-3 | 11.5-E2E-003 (×3) | ✅ GREEN |
| C#5 — missing ETag in 200 OK → `PartUploadError` (retried) | R-P2-3 | 11.5-E2E-004 (×3) | ✅ GREEN |
| C#6 — garbage ETag → `CompleteUploadError` (InvalidPart) | R-P2-3 | 11.5-E2E-005 (×3) | ✅ GREEN |
| C#7 — two parts fail simultaneously, independent retry | R-P2-3 | 11.5-E2E-006 (×3) | ✅ GREEN |
| C#8 — abort during backoff wins (`raceFirst`) | R-P2-3 | 11.5-E2E-007 (×3) | ✅ GREEN |
| C#12 — slow 3G: no premature client timeout | R-P2-9 (BUS, MEDIUM) | 11.5-E2E-008 (×3) | ✅ GREEN |
| C#13 — high latency: abort stays responsive | R-P2-9 | 11.5-E2E-009 (×3) | ✅ GREEN |
| C#15 — slow-loris: part completes (`partTimeout` Epic 13) | R-P2-9 | 11.5-E2E-010 (×3) | ✅ GREEN |
| C#18 — abort during `/initiate` (orphan documented) | R-P2-3 | 11.5-E2E-011 (×3) | ✅ GREEN |
| C#19 — abort between parts (partial, never completed) | R-P2-3 | 11.5-E2E-012 (×3) | ✅ GREEN |
| C#20 — abort during `/complete` (no recovery, Epic 13) | R-P2-3 | 11.5-E2E-013 (×3) | ✅ GREEN |

**Story 11.5 coverage: 13/13 = 100% ✅** (39 runs, 3 engines, no flakes). No lib fix. Epic 13 candidates surfaced: `partTimeout` option (slow-loris C#15), orphan-multipart cleanup on `/initiate` abort (C#18), clean late-stage `/complete`-abort recovery (C#20), event-stream flush-before-error.
...
**Status:** IN-PROGRESS (6/7 stories `done`; only 11.4 remains)
...
**Sub-gate for Story 11.4:** ✅ PASS — 7/7 PW-UI persona specs GREEN × 3 engines = **21 runs, no flakes, no skips** (D3 stabilization satisfied first try — no `@flaky` demotion). Full triptyque green (build + core 204 + adapters 55 + integration 23 + typecheck 5/5) + chaos-isolation re-audited 150/150. **R-P2-1 (DATA HIGH — resume after MinIO TTL)** locked by C2 (reconcile→`ReconcileError`, object absent; current-behaviour lock + Epic 13 auto-re-init candidate); **R-P2-10 (BUS MEDIUM — getProgress foot-gun)** locked by B5. A1/A4 form the tuning-need → tuned-recovery arc; B1 locks the observable forgot-await rejection; B6 locks custom retrySchedule end-to-end; A2 locks throttle-survival cross-engine (CDP gated to Chromium). No lib fix — 3 query-param test-app debug toggles added. **Independent Opus code-review: Approve 0H/0M/3L** (all informational; reviewer traced B5/C2/A4/A1/B6/B1 against lib internals and confirmed no tautologies; advised no change on all 3 LOWs). Status → `done`.
...
**All 7 Epic 11 stories `done` (89/89 = 100%).** Story 11.4 (PW-UI personas) approved by independent Opus review (0H/0M/3L, no change) — §2.9 + §3.8 appended, §1 totals at 89/89. **Remaining: the Epic 11 retrospective** (sets the epic-level gate decision and rolls `gate_decision` to PASS).
