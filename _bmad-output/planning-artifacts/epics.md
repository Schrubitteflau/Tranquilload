...
**Then** the lib detects the missing part and re-uploads it instead of dead-ending. Would flip locking test 11.3-INT-005 (F#14) from "surfaces as CompleteUploadError at complete phase" to "re-uploads and completes". **🔻 DEFERRED from Story 13.2 (2026-06-12, Project Lead, via API-validation pass)** — NOT a quick-win flip: memory-safety tension (reconciled chunks are discarded after the skip and the source is drained by the complete phase, so re-upload requires unbounded retention) + a protocol-agnostic-detection problem (the core can't identify which part S3's `InvalidPart` refers to without parsing S3 error strings). **✅ RESOLVED 2026-06-22 (Story 13.7 spike → decline/document-only, Project Lead via design pass + AskUserQuestion).** The literal AC is not honestly deliverable: granular detection requires a re-probe (already caller-side) and memory-safe single-pass re-upload of an arbitrary GC'd part is impossible without unbounded retention (bounded retention only covers a GC'd part in the retained window — not the general case). The recovery capability already exists caller-side (verify-before-skip in `reconcileCompletedParts`, or catch `CompleteUploadError` → re-probe → re-invoke with a fresh source). 13.7 ships NO library code — it documents the trust boundary + remedies (README "Reconciled-part integrity" + `reconcileCompletedParts` TSDoc). 11.3-INT-005 stays a LOCK (re-tagged, NOT flipped) — the documented boundary. See `13-7-reconciled-part-integrity-and-re-upload.md`.
...
**Total Epic 13 scope:** stories flipping ~13 discrete locking tests (plus the 11.5 events-empty cross-cutting workaround and the 11.7 S3-resume gap) from "documents the gap" to "validates the fix". 1 candidate (F#42 10k-part) is realized as a caller-side/S3-adapter helper per the protocol-agnostic-core rule. **Quick-win tier:** 13.1, 13.2, 13.4 (flip-the-lock). **Spike-gated:** 13.3, 13.5, 13.6 (API-validation/design pass precedes implementation). **Status (2026-06-22):** 13.1/13.2/13.3/13.4 done + released; **13.5 done** (flush half); **13.5b resolved** (ingest-checksum spike → decline/document-only, doc-only, no library code — the transport-checksum win is already caller-achievable via the `chunk` handed to `uploadPart`; F#70 stays a green trust-boundary lock, not flipped); **13.6 done + RELEASED** (adapters 0.1.6; fork (1) opt-in size-bounded auto-buffer, adapter-only patch; `11.7-E2E-002` re-tagged + `13.6-INT-001..007` net-new unit locks; independent Opus review Approve-with-nits 0H/0M/3L, F3 README caveat applied); **13.7 resolved** (reconciled-part integrity spike → decline/document-only, doc-only, no library code — granular detection needs a re-probe and memory-safe single-pass re-upload of an arbitrary GC'd part is impossible without unbounded retention; the recovery is already caller-achievable via verify-before-skip or catch-`CompleteUploadError`→re-probe→re-invoke; 11.3-INT-005/F#14 stays a green lock, not flipped). **All Epic 13 stories done/resolved — only the optional retro remains.** Every new option is opt-in — no default behaviour changes. Epic 12 (circuit-breaker, R-P2-11) tracked separately.
